...
function checkUsername() {
	...
	if (!usernameVal || usernameVal.replace(/\s+/g,"") == "") {
		document.getElementById("usernameMeg").innerHTML = "<font color='red'>用户名不能为空！</font>";
		return;
	}
	if (usernameVal.length < 4 || usernameVal.length > 20 ) {
		document.getElementById("usernameMeg").innerHTML = "<font color='red'>用户名必须为4-20位！</font>";
		return;
	}
	  ...
	  */
	  ...
	  xmlHttp.open("GET", "${pageContext.request.contextPath }/user_ajaxCheckUsername.action?" + "&username=" + 
			  username.value, true);
	  xmlHttp.send(null);
	  xmlHttp.onreadystatechange = function() {
		  if ( xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			  //获得响应，并输出在页面
			  document.getElementById("usernameMeg").innerHTML=xmlHttp.responseText;
		  }
	  };
}
...
function change() {
	...
	captchaImage.src="${pageContext.request.contextPath}/checkImg.action?time=" + new Date().getTime();
}
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<s:actionerror />
					<form id="registerForm" action="${pageContext.request.contextPath }/user_regist.action" method="post" novalidate="novalidate" onsubmit="checkForm()">
						<table>
							<tbody><tr>
								<th>
								...
								<td>
									...
									<span id="usernameMeg"></span><span><s:fielderror fieldName="username"></s:fielderror></span>
								</td>
							</tr>
							<tr>
								<th>
							...
							<tr>
								...
								<td>
									<input type="password" name="repassword" id="repassowrd" class="text" maxlength="20" autocomplete="off">
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									...
									<tr>
										...
										<td>
											<input type="text" name="addr" id="addr" class="text" maxlength="200">
										</td>
									</tr>
								<tr>
									<th>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="verifyCode" name="verifyCode" class="text" maxlength="4" autocomplete="off">
